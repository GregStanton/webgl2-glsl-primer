...
<details> <summary><strong>Q:</strong> The <i>graphics pipeline</i> (a.k.a. <i>rendering pipeline</i>) consists of which two interacting sequences?</summary> <p><strong>A:</strong> The <strong>coordinate pipeline</strong> (mathematical spaces) and the <strong>execution pipeline</strong> (hardware stages).</p> </details>
...
<details>
<summary><strong>Q:</strong> In WebGL, what does the term <i>binding</i> mean?</summary>
...
<p><strong>A:</strong> Suppose you want to draw a rectangle from <em>four</em> vertices using <code>gl.TRIANGLES</code> mode. If you use two triangles, storing three vertices for each, that's <em>six</em> sets of vertex attributes, duplicating data. An element array buffer stores a sequence of vertex indices (e.g., <code>[0, 1, 2, 0, 2, 3]</code>), allowing you to reuse vertices without duplicating them in memory.</p>
<p><strong>Note:</strong> We will compare this element-array-buffer approach to alternative approaches separately.</p>
</details>
